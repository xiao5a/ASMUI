(function(mui, owner) {
	...
	owner.addEvent = function() {
		...
		mui(".mui-scroll").on('tap', '.mui-card', function() {
			...
			mui.openWindow({
				url: '../sfcj/sfcjAdd.html',
				id: "../sfcj/sfcjAdd.html",
				extras: {
					myData: _data
				}
			});
	...
	owner.getsfcjList = function() {
		var param = {};
		param.QY_NAME =  $("#searchid").val();
		
		mui.showLoading("操作中...");
		mui.ajax(appUrl + '/com/neusoft/mle/app/action/sfcj/sfcj.getsfcjList.svc', {
			data: {
				condition: param,
			...
			success: function(data) {
				console.log("---getsfcjList---:" + JSON.stringify(data));
				mui.hideLoading();
				//判断返回状态码，200-ture
				var stFlag = baseUtils.checkStatus(data);
				if(stFlag) {
