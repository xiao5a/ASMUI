...
var UserInfoUtils = {
	...
	setUserInfo:function(userInfo){
		...
		// 'djxh':'20124200100006402173',登记序号
		// "nsrmc": "李刚",  名称
		// "nsrsbh": "422801195811120210",身份证号
		// "token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9",
		// "type":1,
		//" sfzjlx":'201',身份证件类型
		//"yddh": "13898868742" 移动电话
		//    }
	...
	//跳转到登录页面
	gotoLogin:function(gotoUrl,extras){
		mui.openWindow({
...
var $http = {
	...
	get: function(url,data,callback,head=this.headers,time=this.timeout){
		mui.ajax({
			url: url,
			type: "get",
			dataType:'json',
...
var tranIdInfo ={
	...
	"aliauth" : "com.neusoft.login.aliauth" ,//手机端支付宝登录
...
var pageUtils = {
	...
	//设置状态栏
	setStatusbar:function(preHeight=0){
		//设置沉浸式
		//获取状态栏的高度
	...
	//设置返回界面
	back: function(urlId='../app/allapp.html',isReload=false){
		console.log("返回页面>>>"+urlId)
		var old_back = mui.back;
		mui.back = function() {
			pageUtils.popToTargetPage(urlId,isReload,"","")
			old_back();
		}
	},
	/**
	* 从当前页面pop到目标页面
	* targetId 目标页面ID,为mui.openWindow中的id值
	* isReload 是否让目标页面执行reload方法刷新（全局刷新）
	* eventName 自定义事件名称，如果存在，就触发（可以实现局部刷新）
	* data json格式的数据（自定事件参数）
	*/
	popToTargetPage:function(targetId, isReload, eventName, data){
		//获取目标页面
