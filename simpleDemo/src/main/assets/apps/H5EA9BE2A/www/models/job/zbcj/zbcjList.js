(function(mui, owner) {
	...
	owner.addEvent = function() {
		...
		mui(".mui-scroll").on('tap', '.mui-card', function() {
			...
			mui.openWindow({
				url: '../common/map-position.html',
				id: "../common/map-position.html",
				extras: {
					myData: _data
				}
			});
		...
		window.addEventListener('setPositionInfo', function(event) {
			owner.updLocation(event.detail);
		});
	}
	...
	owner.delsfcj = function(obj) {
		var param = {};
		param.ID = obj.ID;

		mui.showLoading("操作中...");
		mui.ajax(appUrl + '/com/neusoft/mle/app/action/sfcj/sfcj.delsfcj.svc', {
			data: {
				condition: param
			},
			dataType: 'json',
			...
			success: function(data) {
				console.log("---delsfcj---:" + JSON.stringify(data));
				mui.hideLoading();
				//判断返回状态码，200-ture
				var stFlag = usercookies.checkStatus(data);
				if(stFlag) {
					var code = data.code;
					if(code != "1") {
	...
	owner.getXkzListAll = function() {
		var param = {};
		param.QY_NAME =  $("#searchid").val();
		
		mui.showLoading("操作中...");
		mui.ajax(appUrl + '/com/neusoft/mle/app/action/common/common.getXkzListAll.svc', {
			data: {
				condition: param,
			...
			success: function(data) {
				console.log("---getXkzListAll---:" + JSON.stringify(data));
				mui.hideLoading();
				//判断返回状态码，200-ture
	...
	owner.updLocation = function(d) {
		console.log("d:" + JSON.stringify(d));
		var param = owner.checkParam(d);
		if (null == param){
			return;
		}

		mui.showLoading("操作中...");
		mui.ajax(appUrl + '/com/neusoft/mle/app/action/common/common.updLocation.svc', {
			data: {
				condition: param
			},
			dataType: 'json',
			...
			success: function(data) {
				console.log("---updLocation---:" + JSON.stringify(data));
				mui.hideLoading();
	...
	owner.checkParam = function(d) {
		var param = {};
		param.UNISCID = d.myData.UNISCID;
		param.XKZBH = d.myData.XKZBH;
		param.QY_CATEGORY_CODE = d.myData.QY_CATEGORY_CODE;		
		param.GPS_LNG = d.NS_LOCATIONS.gps_lng + "";	
		param.GPS_LAT = d.NS_LOCATIONS.gps_lat + "";
		param.BD_LNG = d.NS_LOCATIONS.lng + "";
		param.BD_LAT = d.NS_LOCATIONS.lat + "";
		param.BD_ADDR = d.NS_LOCATIONS.titile;
