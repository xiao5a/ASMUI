(function(mui, owner) {
	...
	owner.addEvent = function() {
		...
		mui(".mui-scroll").on('tap', '.mui-card', function() {
			...
			mui.openWindow({
				url: '../zwcn/zwcnAdd.html',
				id: "../zwcn/zwcnAdd.html",
				extras: {
					myData: _data
				}
			});
	...
	owner.getZwcnXxList = function() {
		var param = {};

		mui.showLoading("操作中...");
		mui.ajax(appUrl + '/com/neusoft/mle/app/action/zwcn/zwcn.getZwcnXxList.svc', {
			data: {
				condition: param,
			...
			success: function(data) {
				console.log("---getZwcnXxList---:" + JSON.stringify(data));
				mui.hideLoading();
				//判断返回状态码，200-ture
	...
	owner.deleXkzbhXx = function(obj) {
		var param = {};
		param.qy_category_code = obj.QY_CATEGORY_CODE;
		param.xkzbh = obj.XKZBH;
		mui.showLoading("操作中...");
		mui.ajax(appUrl + '/com/neusoft/mle/app/action/zwcn/zwcn.deleXkzbhXx.svc', {
			data: {
				condition: param
			},
			dataType: 'json',
			...
			success: function(data) {
				console.log("---deleXkzbhXx---:" + JSON.stringify(data));
				mui.hideLoading();
				//判断返回状态码，200-ture
				var stFlag = baseUtils.checkStatus(data);
				if(stFlag) {
