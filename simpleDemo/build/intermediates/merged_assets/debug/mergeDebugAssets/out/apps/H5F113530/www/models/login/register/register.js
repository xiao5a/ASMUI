(function(mui, owner) {
	...
	owner.registerUser = function() {
		...
		param.certificate_no = document.getElementById("certificate_no").value;
		...
		if (baseUtils.isEmpty(param.name) ){
			mui.toast("请您输入注册人姓名！");
			return;
		}
		
		if (baseUtils.isEmpty(param.certificate_no) ){
			mui.toast("请您输入身份证号！");
			return;
		}
		
		if (!baseUtils.identityCodeValid(param.certificate_no) ){
			return;
		}
		...
		mui.ajax(appUrl + '/com/neusoft/mle/app/action/system/system.registerUser.svc', {
			data: {
				condition: param
			},
			dataType: 'json',
